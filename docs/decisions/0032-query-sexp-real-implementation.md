# 0032 — G122: Real S-expression matching in `query` subcommand

- Status: Accepted
- Date: 2026-06-14
- Scope: `src/commands/query.rs`, `Cargo.toml`, `tests/cli_v019_query_sexpr.rs`
...
`tree-sitter` 0.26.9 já estava disponível via `tree-sitter-language-pack`
1.8. Faltava apenas rotear padrões S-expression para
`tree_sitter::Query::new` + `QueryCursor`.

## Decisão

Adicionar detecção automática de S-expression no pattern, e rotear para o
caminho real do `tree_sitter` quando detectado:

1. Novo enum `QueryType { KindFilter, SExpression }` em
   `src/commands/query.rs`.
2. Nova função `classify_pattern(pattern) -> QueryType` que detecta S-expression
   pela presença de `(`, `)`, ou `@`.
3. Nova função `walk_sexpr` que compila o pattern via
   `tree_sitter::Query::new`, executa via `QueryCursor::matches`, e emite
...
5. Adicionar `tree-sitter = "0.26"` como dependência direta em
   ...
   próprio).
6. O `walk_sexpr` re-parseia a fonte via um `tree_sitter::Parser` direto
...
- **Forçar kind-filter para `SExpression` ser default**: rejeitado
  porque quebraria o pattern v0.1.12 `atomwrite --query function_item`
  que é usado em pipelines existentes.

## Consequências

- Pattern com `(`, `)`, ou `@` agora é roteado para `tree_sitter::Query::new`.
  Erros de parsing S-expression (e.g. `(unclosed`) retornam exit 1 com
  mensagem `invalid S-expression pattern: ...` (via `anyhow::Context`).
...
- 4 novos testes em `tests/cli_v019_query_sexpr.rs`:
  - `sexpr_function_item_returns_main` — pattern
  ...
  - `sexpr_struct_item` — pattern
    `(struct_item name: (type_identifier) @name)` extrai `"Point"`.
  - `kind_filter_still_works` — pattern `function_item` (sem S-expression)
